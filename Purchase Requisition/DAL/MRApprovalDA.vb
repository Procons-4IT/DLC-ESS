...
Public Class MRApprovalDA
    ...
    Public Function MainGridBind(ByVal objEN As PRApprovalEN) As DataSet
        Try
            ...
            objDA.strQuery += " left Outer Join [@Z_RPD1] T1 on T0.DocEntry=T1.DocEntry "
           objDA.strQuery += "   JOIN [@Z_DLC_OAPPT] T3 ON T3.DocEntry = T1.U_Z_ApproveId and (T1.U_Z_AppStatus='P' or T1.U_Z_AppStatus='-')"
            ...
            objDA.strQuery += " And isnull(T2.U_Z_AMan,'N')='Y' AND isnull(T3.U_Z_Active,'N')='Y' and  isnull(T1.U_Z_AppRequired,'N')='Y' and isnull(T0.U_Z_DocStatus,'O')<>'C' and  T2.U_Z_AUser = '" + objEN.UserCode + "' And T3.U_Z_DocType = 'MR'  Order by T0.DocEntry Desc;"
            ...
            objDA.strQuery += "  JOIN [@Z_DLC_APPT2] T2 ON T3.DocEntry = T2.DocEntry "
            objDA.strQuery += " And (T1.U_Z_CurApprover = '" + objEN.UserCode + "' OR T1.U_Z_NxtApprover = '" + objEN.UserCode + "')"
            objDA.strQuery += " And isnull(T2.U_Z_AMan,'N')='Y' AND isnull(T3.U_Z_Active,'N')='Y' and  isnull(T1.U_Z_AppRequired,'N')='Y' and  T2.U_Z_AUser = '" + objEN.UserCode + "' And T3.U_Z_DocType = 'MR' Order by T0.DocEntry Desc;"
    ...
    Public Function BindPRequestApproval(ByVal objEN As PRApprovalEN) As DataSet
        Try
            ...
            objDA.strQuery += " And isnull(T2.U_Z_AMan,'N')='Y' AND isnull(T3.U_Z_Active,'N')='Y' and  isnull(T0.U_Z_AppRequired,'N')='Y' and  T2.U_Z_AUser = '" + objEN.UserCode + "' And T3.U_Z_DocType = 'MR' where T0.DocEntry='" & objEN.DocEntry & "' Order by Convert(Numeric,T0.DocEntry) Desc;"
    ...
    Public Function BindExpenseSummaryApproval(ByVal objEN As PRApprovalEN) As DataSet
        Try
            ...
            objDA.strQuery += "  JOIN [@Z_DLC_APPT2] T2 ON T3.DocEntry = T2.DocEntry "
            objDA.strQuery += " And isnull(T2.U_Z_AMan,'N')='Y' AND isnull(T3.U_Z_Active,'N')='Y' and T2.U_Z_AUser = '" + objEN.UserCode + "' And T3.U_Z_DocType = 'MR' where T0.DocEntry='" & objEN.DocEntry & "' Order by T0.DocEntry Desc;"
            objDA.sqlda = New SqlDataAdapter(objDA.strQuery, objDA.con)
